...
class IQueue{
    ...
    void copyHeap(Node * currTreeNode, Node * otherTreeNode);
    Node * copySubtree(Node * currNode, int counter);
    void preorderTraversal(Node * currNode) const;
    Node * merge(Node * left, Node * right);
    int calcPriorityOfCrop(Crop crop) const;
    void storeCrops(Node * curr, vector<Crop> &theCrop);

    // These functions are for testing purposes
    bool checkMinHeapProperty(Node * currNode) const;
    bool checkMaxHeapProperty(Node * currNode) const;
    bool checkHeapsAreSame(Node * heapOne, Node * heapTwo) const;
